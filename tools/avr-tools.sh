...
AVRDIR="${CMAKE_CURRENT_SOURCE_DIR}/avr"
BOOTLOADER="${CMAKE_CURRENT_SOURCE_DIR}/bootloader/Caterina-Leonardo.hex"
PORT=""
HEXFILE="${CMAKE_BINARY_DIR}/${CMAKE_PROJECT_NAME}.hex"
...
detect_port ()
{
    if [ -d /dev/serial ]; then
        ...
        if ls /dev/serial/by-id/usb-Arduino* 1> /dev/null 2>&1; then
            PORT=$(ls /dev/serial/by-id/usb-Arduino* | head -n1)
